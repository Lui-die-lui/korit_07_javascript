...
// inputCoin으로 살 수 있는 음료 목록들을 집어넣음
...
//살 수 있는 음료 목록 : [object Object],[object Object],[object Object],[object Object]
console.log("살 수 있는 음료 목록 : " + outputList);

for (let i = 0; i < productList.length; i++) {
  console.log(outputList[i]);
}
// 근데 list 내부의 객체들을 하나씩 확인하려고 하니까 이건 출력 됨

/*
 즉 이상의 내용을 정리 해봤을 때, 배열 내부에 객체가 element라면, 배열 자체를 
 콘솔로 출력했을때 결과값이 배열 명으로 형식으로만 나오게 되고,

 각 객체의 명확한 상태를 알기 위해서 결국 배열의 각 element를 돌면서 추출하는 반복문을 작성해야함
*/
...
/*
 outputList 의 element의 자료형은 객체이고, 객체에는 index number 가 존재하지 않으므로 이중 for문을 통해서
 객체 내부의 각 property를 확인하는 것은 불가능합니다.

 그럼 마찬가지로 일반 객체의 각 property를 추출해서 데이터를 가공하는 방식이 불가능
*/
...
// 그래서 해당 key에 할당된 value를 추울하고 싶다면 121번 라인과 같은 방식으로 코드를 작성해야 합니다,
for (const key in person) {
  // 그럼 여기서 추출되는 key는 각각 fName / lName / age 일겁니다.
  console.log(key);
  console.log(person[key]);
  // 이 시점에서 첫 반복이라고 가정해보겠습니다.
  // 그러면 key = 'fName'이 저장되어 있다고 볼 수 있음
}
...
// 그래서 굳이 # 1, 2 로 두 가지 방식을 통해서 객체의 value를 조활 수 있었습니다.
...
// 객체의 property의 key 추출 가능하니 향상된 for 문 처럼 여겨져서
// 배열을 for-in 문으로 추출 시도
for (let num in nums) {
  console.log(num);
...
// 3. for-of 문
// for-in 문의 경우는 객체 한정으로 생각하시는 편이 낫다
// 일반적인 자바의 향상된 for문에 해당하는것은 이것
